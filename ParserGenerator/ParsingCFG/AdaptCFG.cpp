...
void AdaptCFG::eliminateLeftRecursion() {
    for (auto& rule: rules) {
        ...
        if (hasLeftRecursion.empty()) {
            ...
            for (const auto& rhs: noLeftRecursion) {
            ...
            for (const auto& rhs: hasLeftRecursion) {
                std::vector<Definition *> newRhs = rhs;
        ...
        for (auto& leftFactoredRule: leftFactoredRules) {
            if (leftFactoredRule.second.size() > 1) {
                ...
                for (auto& rhs2: leftFactoredRule.second) {
                    std::vector newRhs(rhs2.begin(), rhs2.end());
                    if (newRhs.empty()) {
                ...
                std::vector newRhs = {
                    new Definition(leftFactoredRule.first, leftFactoredRulesMap[leftFactoredRule.first]), newDefinition
                };
                ...
                // just copy it
                std::vector newRhs = {
                    new Definition(leftFactoredRule.first, leftFactoredRulesMap[leftFactoredRule.first])
                };
